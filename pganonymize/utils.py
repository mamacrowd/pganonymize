...
# ---------------------------------------------------------------------------
# Cross-table consistency registry
# ---------------------------------------------------------------------------
# Maps ``(group_id, normalized_original)`` -> anonymized canonical value.
#
# Whenever a provider is invoked for a value, the result is stored here. The
# next time the same ``original_value`` is seen (in the same or a different
# table), the cached anonymized value is returned instead of re-invoking the
# provider — so the same input always produces the same anonymized output
# across the whole DB.
#
# ``group_id`` lets distinct providers share the same registry when they
# produce interchangeable values for the same input (e.g. ``vatnumber``,
# ``fiscalcodebusiness`` and the digit-leading branch of ``fiscalcodevat``
# all share the 9-digit core).
ANONYMIZED_BY_KEY = {}

# Providers excluded from the consistency registry: they intentionally produce
# fresh output on every call (random tokens) or have no meaningful input.
_NON_CONSISTENT_PROVIDERS = {'uuid4', 'apikey', 'clear'}

# Providers that share the "person fiscal code" registry slot.
_FISCAL_PERSON_PROVIDERS = {'fiscalcode'}

# Providers that share the "business fiscal code / VAT" registry slot.
_FISCAL_BUSINESS_OR_VAT_PROVIDERS = {'vatnumber', 'fiscalcodebusiness'}

# Table-ordering constraints: every consumer table must appear AFTER its
# publisher in the YAML ``tables`` list, otherwise the registry is empty when
# the consumer needs to look up a value. Violations abort ``anonymize_tables``.
TABLE_ORDERING_CONSTRAINTS = [
    ('user_profile', ['sim_account', 'cdt_account']),
    ('business_profile', ['anchor_investor']),
]


def _consistent_group_for(provider_name, raw_value):
    """
    Return the registry ``group_id`` for ``(provider_name, raw_value)``, or
    ``None`` if the provider is not consistency-tracked.

    ``fiscalcodevat`` is split by the first character of the raw value:
    digit-leading → business/VAT slot; letter-leading → person slot.
    """
    if not provider_name or provider_name in _NON_CONSISTENT_PROVIDERS:
        return None
    if provider_name in _FISCAL_PERSON_PROVIDERS:
        return 'fiscal_person'
    if provider_name in _FISCAL_BUSINESS_OR_VAT_PROVIDERS:
        return 'fiscal_business_or_vat'
    if provider_name == 'fiscalcodevat':
...
def _to_canonical(provider_name, anonymized_value):
    """Strip provider-specific framing from an anonymized output (e.g. ``IT`` prefix on VATs)."""
    if anonymized_value is None:
        return None
    if provider_name == 'vatnumber':
        s = str(anonymized_value).strip().upper()
        return s[2:] if s.startswith('IT') else s
    return anonymized_value


def _from_canonical(provider_name, canonical_value):
    """Re-apply provider-specific framing to a canonical registry value."""
    if canonical_value is None:
        return None
    if provider_name == 'vatnumber':
        return f'IT{canonical_value}'
    return canonical_value


def _assert_yaml_ordering(definitions):
    """
    Verify that every consumer table in ``TABLE_ORDERING_CONSTRAINTS`` appears
    after its publisher in the YAML schema.

    Raises ``ValueError`` on violation: continuing would silently produce
    inconsistent anonymized values for the affected rows.
    """
    table_names = [list(d.keys())[0] for d in definitions]
    positions = {name: i for i, name in enumerate(table_names)}
    for publisher, consumers in TABLE_ORDERING_CONSTRAINTS:
        if publisher not in positions:
            continue
        for consumer in consumers:
            if consumer in positions and positions[consumer] < positions[publisher]:
                raise ValueError(
                    f"YAML schema ordering violation: '{consumer}' must come after '{publisher}' "
                    f"in the 'tables' list (publishers must precede consumers for the consistency "
                    f"registry to work)."
                )
...
def build_and_then_import_data(connection, table, primary_key, columns,
    ...
    select_columns = [primary_key] + column_names
...
def _invoke_provider(provider_class, orig_value, row, provider_config):
    """Single entry-point for ``alter_value``: passes ``row`` only when the provider needs it."""
    if provider_config.get('use_row'):
        return provider_class.alter_value(orig_value, row=row, **provider_config)
    return provider_class.alter_value(orig_value, **provider_config)


def _anonymize_value(provider_class, provider_name, orig_value, row, provider_config):
    """
    Return the anonymized value for ``orig_value``, reusing the registry when the
    provider participates in cross-table consistency.

    A provider participates when ``_consistent_group_for`` returns a non-``None`` group_id.
    The registry stores values in *canonical* form (e.g. VATs without the ``IT`` prefix);
    ``_from_canonical`` re-applies the provider's framing on read.
    """
    group_id = _consistent_group_for(provider_name, orig_value)
    if group_id is None or orig_value is None:
        return _invoke_provider(provider_class, orig_value, row, provider_config)

    key = (group_id, _normalize_key(group_id, orig_value))
    if key[1] is None:
        return _invoke_provider(provider_class, orig_value, row, provider_config)

    cached = ANONYMIZED_BY_KEY.get(key)
    if cached is not None:
        return _from_canonical(provider_name, cached)

    value = _invoke_provider(provider_class, orig_value, row, provider_config)
    canonical = _to_canonical(provider_name, value)
    if canonical is not None:
        ANONYMIZED_BY_KEY[key] = canonical
    return value
...
def get_column_values(row, columns):
    ...
    for definition in columns:
        ...
        value = _anonymize_value(provider_class, provider_config['name'], orig_value, row, provider_config)
